...
def mainpage():
    ...
    # cursor = g.conn.execute("SELECT diet_plan_id,fitness_plan_id FROM plans_consist_of")
    # diet_plan_ids = []
    # fitness_plan_ids = []
    # for result in cursor:
    #     diet_plan_ids.append(result['diet_plan_id'])
    #     fitness_plan_ids.append(result['fitness_plan_id'])
    # cursor.close()
    #
    # cursor = g.conn.execute("SELECT coach_id,plan_duration FROM administration")
    # coach_plan_ids = []
    # plan_durations = []
    # for result in cursor:
    #     coach_plan_ids.append(result['coach_id'])
    #     plan_durations.append(result['plan_duration'])
    # cursor.close()
    #
    # context1 = dict(diet_plan_ids=diet_plan_ids)
    # # print(context)
    # context2 = dict(fitness_plan_ids=fitness_plan_ids)
    # context3 = dict(coach_plan_ids=coach_plan_ids)
    # context4 = dict(plan_durations=plan_durations)
    # return render_template("mainpage.html", **context1, **context2, **context3, **context4)
...
def planselection():
    ...
    return render_template('mainpage.html',a=chosen_diet_plan_id,b=chosen_fitness_plan_id,c=chosen_plan_duration,d=chosen_coach_id)
    # redirect('/mainpage')
